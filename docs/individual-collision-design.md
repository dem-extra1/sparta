...
5. **Retire the regiment circle (in progress, shipping in slices).** Make soldiers
   ...
   - **Slice 1 [DONE] — friendly collision.** A soldier->regiment **coupling** slides each
     ...
     residual is a later tuning refinement). The regiment broadphase precomputes each
     regiment's block extent once per tick (O(n) instead of O(n²) allocations) for
     scalability past ~30 friendly regiments/side (#295).
