...
## Friendly-avoidance steering for the soldier layer (phase 5). Instead of position-
## correcting overlapping bodies, it writes a per-soldier velocity bias into each unit's
## `_sim_steer`, which SoldierBodies feeds forward — so a body damps AWAY from a crowding
## FRIENDLY and drifts off it over a few frames, never snapping. Enemy overlap is
## deliberately ignored here: the fighting resolves it through knockback (SoldierMelee),
## so the standoff emerges from press-in vs knockback rather than a separation rule.
##
## Two tiers of bodies are gathered each tick:
##   - Engaged front ranks (the original tier, always included).
##   - ALL bodies of any regiment whose block overlaps a FRIENDLY regiment's block (the
##     friendly-contact tier, phase 5): two marching friendlies pressing together steer
##     apart even when neither is fighting; the body->regiment coupling then slides the
##     centers off each other.
##
...
# Tuned so crowded ranks slide back to body contact in a fraction of a second, without
# the jitter the old hard position-snap produced.
...
static func accumulate(units: Array, frame: int) -> void:
	...
	# Precompute each regiment's block extent once (O(n)), so _overlaps_friendly can do
	# O(1) lookups instead of recomputing the PackedVector2Array per pair (O(n²) allocs).
	var extents: Dictionary = {}
	for o in sorted_units:
		var u0: Unit = o as Unit
		if u0 != null and u0.state != Unit.State.DEAD:
			extents[u0] = u0.soldier_block_extent()
...
## friendly-contact tier so an uncrowded line costs the same as before. `extents` is the
## precomputed per-regiment block extent (keyed by Unit), so this runs O(n) per call
## without re-allocating PackedVector2Arrays.
static func _overlaps_friendly(u: Unit, sorted_units: Array, extents: Dictionary) -> bool:
	var reach_u: float = extents.get(u, 0.0)
	for o in sorted_units:
		var v: Unit = o as Unit
		if v == null or v == u or v.state == Unit.State.DEAD or v.team != u.team:
			continue
		if u.position.distance_to(v.position) < reach_u + extents.get(v, 0.0):
			return true
	return false
