...
# --- Cosmetic soldier flocking state -------------------------------------------
...
var _combat_clock: float = 0.0          # render-time clock driving the melee churn
...
func _ready() -> void:
	...
	# Polygon2D) layered just under this node via their relative z_index.
	z_index = 3
...
func take_casualties(amount: int, attacker: Unit) -> void:
	...
	# Cosmetic "men fall" markers: drop a small fading heap of bodies on the
...
# Individual-soldier rendering. The regiment is drawn as a formation
...
# Soldier flocking. The marks no longer snap to their formation slot:
...
# Melee churn. While a regiment is FIGHTING, its front-rank soldier marks
...
func _update_flock(delta: float) -> void:
	...
	# frame, so it skips the at-rest fast-path even when the unit is standing
	...
	# mark's depth behind it scales how hard it churns while fighting.
...
## Melee churn offset for one front-rank mark. Returns an offset in the
...
func _draw() -> void:
	...
	# The soldier marks are rendered by the flocking MultiMeshes and the
